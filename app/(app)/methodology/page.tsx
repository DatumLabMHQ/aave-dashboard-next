...
export default async function Methodology() {
  const [s, d] = await Promise.all([platformStatus(), loadAave()]);
  ...
  const status: StatusRow[] = [
    ...
    { icon: 'timer', label: 'Snapshot and refresh cadence', detail: 'Reserves and markets are snapshotted hourly and kept at daily grain; DefiLlama is read daily. These pages revalidate every five minutes.' },
    { icon: 'pulse', label: 'Trend window', detail: `Up to ${config.trend.days} days of daily market rows behind the overview trend (the platform holds Aave from 4 September 2026); ${config.contextDays} days of DefiLlama behind the long view.`, badge: { text: `${d.history.length} days`, tone: 'neutral' } },
          ...
          <CardContent>
            {r ? (
              <div className="grid grid-cols-1 gap-4 @2xl/main:grid-cols-[1fr_1fr_2fr]">
                <div className="rounded-lg border bg-muted/40 p-4"><div className="text-xs text-muted-foreground">Our sum of the reserves</div><div className="mt-1 text-2xl font-medium tracking-tight tabular-nums">{usd(r.ours)}</div><div className="mt-1 text-xs text-muted-foreground">as of {d.asOf}</div></div>
