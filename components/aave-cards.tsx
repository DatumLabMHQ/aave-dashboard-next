// The KPI row: shadcn's section-cards recipe on the Aave overview numbers. Server component.
...
import type { AaveOverview } from '@/lib/aave-types';
...
export function AaveCards({ kpis: k, asOf }: { kpis: AaveOverview['kpis']; asOf: string }) {
  const stats: Stat[] = [
    { label: 'Supplied', value: usd(k.supplied), change: k.suppliedChange7d, headline: k.suppliedChange7d >= 0 ? 'Growing over the week' : 'Shrinking over the week', detail: `${count(k.reserves)} reserves in ${count(k.markets)} markets on ${count(k.chains)} chains, as of ${asOf}` },
    { label: 'Borrowed', value: usd(k.borrowed), change: k.borrowedChange7d, headline: k.borrowedChange7d >= k.suppliedChange7d ? 'Demand outpacing supply' : 'Supply outpacing demand', detail: 'Debt outstanding across every market' },
    ...
    { label: 'On Aave v4', value: pct(k.v4Share, 1), headline: k.v4Share > 25 ? 'v4 is a quarter of the book' : 'v3 still carries the book', detail: `Share of supplied value in v4 spokes; weighted supply APY ${pct(k.supplyApy, 2)}` },
  ];
  return (
